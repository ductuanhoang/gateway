...
#define SENSOR_UPDATE_INTERVAL 10 // ESP_BLE_MESH_SENSOR_NOT_APPL_UPDATE_INTERVAL
...
static int8_t outdoor_temp = 100; /* Outdoor temperature is 100 Degrees Celsius */
...
{
    ESP_LOGI(TAG_MESH, "net_idx 0x%03x, addr 0x%04x", net_idx, addr);
    ESP_LOGI(TAG_MESH, "flags 0x%02x, iv_index 0x%08" PRIx32, flags, iv_index);
                                             ...
                                             esp_ble_mesh_prov_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_PROV_REGISTER_COMP_EVT:
        ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_PROV_REGISTER_COMP_EVT, err_code %d", param->prov_register_comp.err_code);
        break;
    case ESP_BLE_MESH_NODE_PROV_ENABLE_COMP_EVT:
        ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_NODE_PROV_ENABLE_COMP_EVT, err_code %d", param->node_prov_enable_comp.err_code);
        break;
    case ESP_BLE_MESH_NODE_PROV_LINK_OPEN_EVT:
        ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_NODE_PROV_LINK_OPEN_EVT, bearer %s",
                 param->node_prov_link_open.bearer == ESP_BLE_MESH_PROV_ADV ? "PB-ADV" : "PB-GATT");
        break;
    case ESP_BLE_MESH_NODE_PROV_LINK_CLOSE_EVT:
        ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_NODE_PROV_LINK_CLOSE_EVT, bearer %s",
                 param->node_prov_link_close.bearer == ESP_BLE_MESH_PROV_ADV ? "PB-ADV" : "PB-GATT");
        break;
    case ESP_BLE_MESH_NODE_PROV_COMPLETE_EVT:
        ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_NODE_PROV_COMPLETE_EVT");
        prov_complete(param->node_prov_complete.net_idx, param->node_prov_complete.addr,
                      param->node_prov_complete.flags, param->node_prov_complete.iv_index);
        break;
    case ESP_BLE_MESH_NODE_PROV_RESET_EVT:
        ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_NODE_PROV_RESET_EVT");
        break;
    case ESP_BLE_MESH_NODE_SET_UNPROV_DEV_NAME_COMP_EVT:
        ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_NODE_SET_UNPROV_DEV_NAME_COMP_EVT, err_code %d", param->node_set_unprov_dev_name_comp.err_code);
        break;
    default:
        break;
    }
                                              ...
                                              esp_ble_mesh_cfg_server_cb_param_t *param)
{
    if (event == ESP_BLE_MESH_CFG_SERVER_STATE_CHANGE_EVT)
    {
        ...
        case ESP_BLE_MESH_MODEL_OP_APP_KEY_ADD:
            ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_MODEL_OP_APP_KEY_ADD");
            ESP_LOGI(TAG_MESH, "net_idx 0x%04x, app_idx 0x%04x",
                     param->value.state_change.appkey_add.net_idx,
                     param->value.state_change.appkey_add.app_idx);
            ESP_LOG_BUFFER_HEX("AppKey", param->value.state_change.appkey_add.app_key, 16);
            break;
        case ESP_BLE_MESH_MODEL_OP_MODEL_APP_BIND:
            ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_MODEL_OP_MODEL_APP_BIND");
            ESP_LOGI(TAG_MESH, "elem_addr 0x%04x, app_idx 0x%04x, cid 0x%04x, mod_id 0x%04x",
                     param->value.state_change.mod_app_bind.element_addr,
                     param->value.state_change.mod_app_bind.app_idx,
                     param->value.state_change.mod_app_bind.company_id,
                     param->value.state_change.mod_app_bind.model_id);
            // TODO: check save to array save element_addr
            uint8_t ret = user_check_and_add_sub_device(param->value.state_change.mod_app_bind.element_addr);
            if (ret != 0)
                ESP_LOGI(TAG_MESH, "NEW device address added: %d", param->value.state_change.mod_app_bind.element_addr);
            break;
        case ESP_BLE_MESH_MODEL_OP_MODEL_SUB_ADD:
            ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_MODEL_OP_MODEL_SUB_ADD");
            ESP_LOGI(TAG_MESH, "elem_addr 0x%04x, sub_addr 0x%04x, cid 0x%04x, mod_id 0x%04x",
...
static void example_ble_mesh_send_sensor_descriptor_status(esp_ble_mesh_sensor_server_cb_param_t *param)
{
    ...
    if (!status)
    {
        ESP_LOGE(TAG_MESH, "No memory for sensor descriptor status!");
        return;
    }
...
send:
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG_MESH, "Failed to send Sensor Descriptor Status");
    }
    free(status);
}
...
static void example_ble_mesh_send_sensor_cadence_status(esp_ble_mesh_sensor_server_cb_param_t *param)
{
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG_MESH, "Failed to send Sensor Cadence Status");
    }
}
...
static void example_ble_mesh_send_sensor_settings_status(esp_ble_mesh_sensor_server_cb_param_t *param)
{
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG_MESH, "Failed to send Sensor Settings Status");
    }
}
...
static void example_ble_mesh_send_sensor_setting_status(esp_ble_mesh_sensor_server_cb_param_t *param)
{
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG_MESH, "Failed to send Sensor Setting Status");
    }
}
...
static uint16_t example_ble_mesh_get_sensor_data(esp_ble_mesh_sensor_state_t *state, uint8_t *data)
{
    ...
    {
        ESP_LOGE(TAG_MESH, "%s, Invalid parameter", __func__);
        return 0;
    }
...
static void example_ble_mesh_send_sensor_status(esp_ble_mesh_sensor_server_cb_param_t *param)
{
    ...
    esp_err_t err;
    int i;
    extern EndDeviceData_t my_end_device_data_1;
    extern EndDeviceData_t my_end_device_data_2;

    net_buf_simple_add_u8(&sensor_data_0, my_end_device_data_1.status);
    net_buf_simple_add_u8(&sensor_data_1, my_end_device_data_1.status);

    ESP_LOGI(TAG_MESH, "add my_end_device_data_1.status to the data : %d", my_end_device_data_1.status);
    /**
    ...
    if (!status)
    {
        ESP_LOGE(TAG_MESH, "No memory for sensor status!");
        return;
    }
...
send:
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG_MESH, "Failed to send Sensor Status");
    }
    free(status);
}
...
static void example_ble_mesh_send_sensor_column_status(esp_ble_mesh_sensor_server_cb_param_t *param)
{
    ...
    if (!status)
    {
        ESP_LOGE(TAG_MESH, "No memory for sensor column status!");
        return;
    }
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG_MESH, "Failed to send Sensor Column Status");
    }
    free(status);
}
...
{
    ESP_LOGI(TAG_MESH, "Sensor server, event %d, src 0x%04x, dst 0x%04x, model_id 0x%04x",
    ...
    case ESP_BLE_MESH_SENSOR_SERVER_RECV_GET_MSG_EVT:
        ...
        case ESP_BLE_MESH_MODEL_OP_SENSOR_DESCRIPTOR_GET:
            ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_MODEL_OP_SENSOR_DESCRIPTOR_GET");
            example_ble_mesh_send_sensor_descriptor_status(param);
            break;
        case ESP_BLE_MESH_MODEL_OP_SENSOR_CADENCE_GET:
            ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_MODEL_OP_SENSOR_CADENCE_GET");
            example_ble_mesh_send_sensor_cadence_status(param);
            break;
        case ESP_BLE_MESH_MODEL_OP_SENSOR_SETTINGS_GET:
            ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_MODEL_OP_SENSOR_SETTINGS_GET");
        ...
        case ESP_BLE_MESH_MODEL_OP_SENSOR_GET:
            ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_MODEL_OP_SENSOR_GET");
        ...
        case ESP_BLE_MESH_MODEL_OP_SENSOR_COLUMN_GET:
            ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_MODEL_OP_SENSOR_COLUMN_GET");
            example_ble_mesh_send_sensor_column_status(param);
            break;
        case ESP_BLE_MESH_MODEL_OP_SENSOR_SERIES_GET:
            ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_MODEL_OP_SENSOR_SERIES_GET");
            example_ble_mesh_send_sensor_series_status(param);
            break;
        case ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_GET:
            ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET");

            break;
        default:
            ESP_LOGE(TAG_MESH, "Unknown Sensor Get opcode 0x%04" PRIx32, param->ctx.recv_op);
            return;
        }
        break;
    case ESP_BLE_MESH_SENSOR_SERVER_RECV_SET_MSG_EVT:
        ...
        case ESP_BLE_MESH_MODEL_OP_SENSOR_CADENCE_SET:
            ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_MODEL_OP_SENSOR_CADENCE_SET");
            example_ble_mesh_send_sensor_cadence_status(param);
            break;
        case ESP_BLE_MESH_MODEL_OP_SENSOR_CADENCE_SET_UNACK:
            ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_MODEL_OP_SENSOR_CADENCE_SET_UNACK");
            break;
        case ESP_BLE_MESH_MODEL_OP_SENSOR_SETTING_SET:
            ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_MODEL_OP_SENSOR_SETTING_SET");
            example_ble_mesh_send_sensor_setting_status(param);
            break;
        case ESP_BLE_MESH_MODEL_OP_SENSOR_SETTING_SET_UNACK:
            ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_MODEL_OP_SENSOR_SETTING_SET_UNACK");
            break;
        default:
            ESP_LOGE(TAG_MESH, "Unknown Sensor Set opcode 0x%04" PRIx32, param->ctx.recv_op);
            break;
        }
        break;
    ...
    //     }
    default:
        ESP_LOGE(TAG_MESH, "Unknown Sensor Server event %d", event);
        break;
    }
}
                                               ...
                                               esp_ble_mesh_generic_server_cb_param_t *param)
{
    ...
    ESP_LOGI(TAG_MESH, "event 0x%02x, opcode 0x%04x, src 0x%04x, dst 0x%04x",
             event, param->ctx.recv_op, param->ctx.addr, param->ctx.recv_dst);

    switch (event)
    {
    case ESP_BLE_MESH_GENERIC_SERVER_STATE_CHANGE_EVT:
        ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_GENERIC_SERVER_STATE_CHANGE_EVT");
        if (param->ctx.recv_op == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET ||
            param->ctx.recv_op == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET_UNACK)
        {
            ESP_LOGI(TAG_MESH, "onoff 0x%02x", param->value.state_change.onoff_set.onoff);
            // example_change_led_state(param->model, &param->ctx, param->value.state_change.onoff_set.onoff);
        }
        break;
    case ESP_BLE_MESH_GENERIC_SERVER_RECV_GET_MSG_EVT:
        ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_GENERIC_SERVER_RECV_GET_MSG_EVT");
        if (param->ctx.recv_op == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_GET)
        {
            srv = param->model->user_data;
            ESP_LOGI(TAG_MESH, "onoff 0x%02x", srv->state.onoff);
            example_handle_gen_onoff_msg(param->model, &param->ctx, NULL);
        }
        break;
    case ESP_BLE_MESH_GENERIC_SERVER_RECV_SET_MSG_EVT:
        ESP_LOGI(TAG_MESH, "ESP_BLE_MESH_GENERIC_SERVER_RECV_SET_MSG_EVT");
        if (param->ctx.recv_op == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET ||
            param->ctx.recv_op == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET_UNACK)
        {
            ESP_LOGI(TAG_MESH, "onoff 0x%02x, tid 0x%02x", param->value.set.onoff.onoff, param->value.set.onoff.tid);
            if (param->value.set.onoff.op_en)
            {
                ESP_LOGI(TAG_MESH, "trans_time 0x%02x, delay 0x%02x",
        ...
        break;
    default:
        ESP_LOGE(TAG_MESH, "Unknown Generic Server event 0x%02x", event);
        break;
    }
}
                                             ...
                                             esp_ble_mesh_model_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_MODEL_OPERATION_EVT:
        if (param->model_operation.opcode == ESP_BLE_MESH_VND_MODEL_OP_SEND)
        {
            ...
            ESP_LOGI(TAG_MESH, "Recv 0x%06x, tid 0x%04x", param->model_operation.opcode, tid);
            // esp_err_t err = esp_ble_mesh_server_model_send_msg(&vnd_models[0],
            //                                                    param->model_operation.ctx, ESP_BLE_MESH_VND_MODEL_OP_STATUS,
            //                                                    sizeof(tid), (uint8_t *)&tid);
            // if (err)
            // {
            //     ESP_LOGE(TAG_MESH, "Failed to send message 0x%06x", ESP_BLE_MESH_VND_MODEL_OP_STATUS);
            // }
        }
        break;
    case ESP_BLE_MESH_MODEL_SEND_COMP_EVT:
        ...
        {
            ESP_LOGE(TAG_MESH, "Failed to send message 0x%06x", param->model_send_comp.opcode);
            break;
        }
        ESP_LOGI(TAG_MESH, "Send 0x%06x", param->model_send_comp.opcode);
        break;
    default:
        break;
    }
...
static esp_err_t ble_mesh_init(void)
{
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG_MESH, "Failed to initialize mesh stack");
        return err;
    }
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG_MESH, "Failed to enable mesh node");
        return err;
    }

    ESP_LOGI(TAG_MESH, "BLE Mesh sensor server initialized");
...
void user_ble_mesh_init(void)
{
    ...
    if (err)
    {
        ESP_LOGE(TAG_MESH, "esp32_bluetooth_init failed (err %d)", err);
        return;
    }
    ...
    if (err)
    {
        ESP_LOGE(TAG_MESH, "Bluetooth mesh init failed (err %d)", err);
    }
}
/***********************************************************************************************************************
 * static functions
