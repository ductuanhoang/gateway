...
{
    int ret = -1;
    int indx = user_mqtt_client.sub_topic.count;
    if (NULL == subTopic)
    {
    ...
    else if (user_mqtt_getConnetion())
    {
        ...
        if (indx == user_mqtt_client.sub_topic.count)
        {
            if (user_mqtt_client.sub_topic.count >= MQTT_MAX_SUB_TOPIC)
            {
                ESP_LOGE(TAG_MQTT, "mqtt subscribe topic number is max > MQTT_MAX_SUB_TOPIC");
                return -1;
            }
        }
        ...
        ESP_LOGI(TAG_MQTT, "add topic sub %s -- %d",
                 ...
                 user_mqtt_client.sub_topic.count);
...
static esp_err_t mqtt_event_handler(esp_mqtt_event_handle_t event)
{
    ...
    case MQTT_EVENT_DATA:
        ...
        if (user_mqtt_client.buffer_message_length == event->total_data_len)
        {
            ...
            for (int i = 0; i < user_mqtt_client.sub_topic.count; i++)
            {
